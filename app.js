...
// === IMPORT MODULES ===
const { prosesResellerTransaksi } = require('./modules/reseller');
const { attachResellerMenu } = require('./modules/resellerMenu');
const { attachAdminTools } = require('./modules/adminTools');
...
const { kirimNotifGrupTransaksi } = require('./modules/groupNotifier');
...
attachResellerMenu(bot);
attachAdminTools(bot);
...
db.run(`CREATE TABLE IF NOT EXISTS transactions (
  ...
  if (err) {
    logger.error('❌ Gagal membuat tabel transactions:', err.message);
  }
});

// === CREATE TABEL UTAMA (1x saja) ===
db.serialize(() => {
  db.run(`CREATE TABLE IF NOT EXISTS users (
    user_id INTEGER PRIMARY KEY,
    saldo INTEGER DEFAULT 0,
...
async function sendMainMenu(ctx) {
  const keyboard = [
    ...
    [
      { text: '👥 Grup Telegram', url: 'https://t.me/+HIJr9u1KjZwwYjg1' }
    ]
  ];
...
// Debug
console.log(`🗓️ Tanggal: ${tanggal}`);
console.log(`⏱️ Uptime : ${uptimeFormatted} (${seconds}s)`);
...
if (topUsers.length > 0) {
  topUsersText = '\n🏆  *_Top 3 Pengguna Aktif_ :*\n' + 
    topUsers.map((user, index) => {
      const username = escapeMarkdownV2(user.username || 'Unknown User');
      const rankEmoji = ['🥇','🥈','🥉'][index] || '⭐';
      return `${rankEmoji} \\- ${username} \\(${user.transaction_count} transaksi\\)`;
    }).join('\n');
}
...
let messageText = `
╭━━━❖ *VPN AUTO PANEL* ❖━━━╮
┃ 🏪 _*Store*_      : ${escapeMarkdownV2(NAMA_STORE)}
┃ ${statusEmoji} *Status*    : Online ${spinner}
┃ 🗓️ *Tanggal* : ${escapeMarkdownV2(tanggal)}
┃ 💳 _Pembayaran via QRIS_
┃
┃ 📊 *Bot Status :*
┃ ┣ ⏱️ *Uptime* : ${escapeMarkdownV2(uptimeFormatted)}
...
// === EXAMPLE MAIN HANDLER (create akun) ===
bot.command('createvpn', async (ctx) => {
  const args = ctx.message.text.split(' ');
  if (args.length !== 4) {
    return ctx.reply('Format: /createvpn <type> <username> <exp>');
  }
  const [cmd, type, username, expStr] = args;
  const exp = parseInt(expStr);

  db.get('SELECT saldo, role, reseller_level FROM users WHERE user_id = ?', [ctx.from.id], async (err, user) => {
    if (err || !user) return ctx.reply('❌ Gagal ambil data user.');

    const serverHarga = 10000; // Misal harga fixed
    let hargaSatuan = serverHarga;

    if (user.role === 'reseller') {
      const result = await prosesResellerTransaksi(ctx, user, serverHarga, exp, type, username);
    ...
    return ctx.reply(`✅ Akun ${type.toUpperCase()} ${username} berhasil dibuat selama ${exp} hari. Harga: Rp${totalHarga}`);
  });
});
...
bot.action('trial_ssh', async (ctx) => {
  ...
  db.get('SELECT has_trial FROM users WHERE user_id = ?', [userId], (err, row) => {
    if (err || !row) return ctx.reply('❌ Gagal cek status trial.');
    ...
    exec('bash ./scripts/trialssh.sh', (error, stdout, stderr) => {
      if (error) return ctx.reply('❌ Gagal jalankan script trial.');
...
bot.action('trial_vmess', async (ctx) => {
  ...
  db.get('SELECT has_trial FROM users WHERE user_id = ?', [userId], (err, row) => {
    if (err || !row) return ctx.reply('❌ Gagal cek status trial.');
    if (row.has_trial === 1) return ctx.reply('😅 Kamu sudah pernah ambil trial bro.');
      ...
      db.get('SELECT saldo FROM users WHERE user_id = ?', [ctx.from.id], async (err, user) => {
        ...
        try {
          if (action === 'create') {
            ...
            // 🔒 Cek akun valid dulu sebelum renew
          ...
          // ✅ Potong saldo & update jumlah akun untuk create
          db.run('UPDATE users SET saldo = saldo - ? WHERE user_id = ?', [totalHarga, ctx.from.id]);

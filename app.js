...
// Import semua trial route
app.use(require('./routes/trialssh'));
app.use(require('./routes/trialvmess'));
app.use(require('./routes/trialvless'));
app.use(require('./routes/trialtrojan'));
app.use(require('./routes/trialshadowsocks'));
...
async function sendMainMenu(ctx) {
  ...
  [
    { text: '🛡 Comingsoon', callback_data: 'service_trial' },
...
async function handleServiceAction(ctx, action) {
  ...
  keyboard = [
    [{ text: '♻️ SSH & OpenVPN', callback_data: 'renew_ssh' }],
    [
        ...
        db.get('SELECT saldo FROM users WHERE user_id = ?', [ctx.from.id], async (err, user) => {
          ...
          if (action === 'create') {
            ...
            if (type === 'vmess') {
              ...
              msg = await trialtrojan(username, exp, quota, iplimit, auth, serverId);
            } else if (type === 'shadowsocks') {
              msg = await trialshadowsocks(username, exp, quota, iplimit, auth, serverId);
            } else if (type === 'ssh') {
              msg = await trialssh(username, exp, iplimit, serverId);
            }
          } else if (action === 'renew') {
            if (type === 'vmess') {

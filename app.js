...
{ text: '☎ Admin', url: 'https://t.me/joysvpnstore' }
    ],
  ];
...
async function handleDepositState(ctx, userId, data) {
  ...
  if (data === 'delete') {
    ...
    if (parseInt(currentAmount) < 500) {
      return await ctx.answerCbQuery('⚠️ Jumlah minimal top-up adalah  500 Ya Kak...!!!', { show_alert: true });
    }
...
async function processDeposit(ctx, amount) {
  ...
  try {
    ...
    const inlineKeyboard = [
      [
        {
          text: "🏧 Penerima Transfer", 
          url: "https://t.me/joysvpnstore"
        }
      ]
    ];

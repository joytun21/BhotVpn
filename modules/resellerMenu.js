// modules/resellerMenu.js const db = require('../database');

function attachResellerMenu(bot) { bot.command('komisi', (ctx) => { const userId = ctx.from.id; db.get('SELECT role, reseller_level FROM users WHERE user_id = ?', [userId], (err, user) => { if (err || !user || user.role !== 'reseller') { return ctx.reply('❌ Kamu bukan reseller.'); }
...
bot.command('riwayatreseller', (ctx) => { const userId = ctx.from.id; db.get('SELECT role FROM users WHERE user_id = ?', [userId], (err, user) => { if (err || !user || user.role !== 'reseller') { return ctx.reply('❌ Kamu bukan reseller.'); }
...
bot.command('topreseller', (ctx) => { db.all( SELECT u.user_id, u.reseller_level, SUM(r.komisi) AS total_komisi FROM reseller_sales r LEFT JOIN users u ON r.reseller_id = u.user_id GROUP BY r.reseller_id ORDER BY total_komisi DESC LIMIT 10, [], async (err, rows) => { if (err || !rows.length) return ctx.reply('❌ Gagal ambil top reseller.');

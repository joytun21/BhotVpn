// modules/reseller/logic/tambah.js
module.exports = (bot, db, adminIds) => {
  ...
  bot.on('text', async (ctx) => {
    ...
    if (isNaN(resellerId)) {
      return ctx.reply('⚠️ Format ID tidak valid. Kirim angka ID Telegram yang valid.');
    }

    db.run(
      'INSERT OR IGNORE INTO reseller_users (user_id, saldo) VALUES (?, 0)',
      [resellerId],
      (err) => {
        if (err) {
          console.error('❌ DB error saat tambah reseller:', err.message);
